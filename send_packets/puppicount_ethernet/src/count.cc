...
void count(const ap_uint<64> candin, ap_uint<64> &candout, ap_uint<48> src, ap_uint<48> dest,
        ...
        static ap_uint<67> buffer[512];
        //static ap_uint<8> wrptr = 0, rdptr = 0, headerloc, npptr = 0, nprdptr = 0;
        static ap_uint<9> wrptr = 0, rdptr = 0, npptr = 0, nprdptr = 0;
        static ap_uint<12> npbuffer[512] = {0}, npuppi = 0; //npbuffer[256]
        ...
        if (write && valid) {
            prep(66,3) = candin;
            prep[0] = 0;
            if (lastin && npuppi > 4) prep[1] = 1; //if npuppi <= 4, will pad then mark lastout
            else prep[1] = 0;
            if (firstin) prep[2] = 1;
            else prep[2] = 0;
            buffer[wrptr] = prep;
        ...
        if (eventsready > 0) {
            ...
            if (buffer[rdptr][2]) { //event header
