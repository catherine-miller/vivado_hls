...
void generateevent(ap_uint<64> v[NPUPPI], unsigned int &npuppi, uint64_t &header) {
   ...
   /* if (option == 0 || option == 1) {
	...
	for (uint i = 0; i < last; ++i) {
        if (rand() % 30 == 0) { //zeroing out some candidates
            //v[i] = 0;
            //npuppi -= 1;
...
int main() {
    ...
    srand(25);
    int u1 = 0, u2 = 0;
    for (unsigned int n = 0; n < NTESTS; ++n) { //one event is sent every iteration
        for (unsigned int i = 0; i < 54; ++i) { //one clock cycle
            if (i == 0) {
            ...
            if (check) {
                ...
                if (firstalgo xor lastalgo) flip = !flip; //check that number of firstalgo and lastalgo are the same
                ...
                if (first1 xor last1) flip1 = !flip1; //check that number of firstalgo and lastalgo coming from "streamv" are the same
